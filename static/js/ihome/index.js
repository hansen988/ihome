...
$(document).ready(function(){
    ...
    $(".register-login").show();
    // 检查用户的登录状态
    $.get('/api/v1.0/get_login', function (resp) {
        if (resp.errno == "0") {
            // 取数据进行判断是否有值
    ...
    // 获取幻灯片要展示的房屋基本信息
    $.get("/api/v1.0/taocans/index", function (resp) {
        if (resp.errno == "0") {
            $(".swiper-wrapper").html(template("swiper-houses-tmpl", {"taocans": resp.data}))
