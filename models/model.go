...
func (this *House) To_one_house_desc() interface{} {
	...
	orders := []OrderHouse{}
	o := orm.NewOrm()
	order_num, _ := o.QueryTable("order_house").Filter("house__id", this.Id).Filter("status", ORDER_STATUS_COMPLETE).OrderBy("-ctime").Limit(10).All(&orders)
	// if err != nil {
...
func init() {
	...
	fmt.Println(utils.G_mysql_dbname)
	//注册model
